...
class ConfigManager:
    ...
    def __init__(
        ...
        self.base_dir: Path = base_dir or Path(os.getenv("IPGEO_HOME", str(Path.home() / ".ipgeo")))
        ...
        self.maxmind_credentials: tuple[str | None, str | None] = (
            os.getenv("MAXMIND_ACCOUNT_ID"),
            os.getenv("MAXMIND_LICENSE_KEY"),
        )

        # New: Redis and local DB paths
        self.redis_url: str = os.getenv("IPGEO_REDIS_URL", "redis://localhost")
        env_mmdb_path = os.getenv("IPGEO_MMDB_PATH")
        self.maxmind_db_path: Optional[Path] = (
            Path(env_mmdb_path) if env_mmdb_path is not None else None
        )
    ...
    def get_maxmind_auth(self) -> tuple[str | None, str | None]:
    ...
    def _load(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._handle_broken_config(exc)
    ...
    def _handle_broken_config(self, exc: Exception) -> Dict[str, Any]:
        logger.warning(
    ...
    def _merge_dicts(base: Dict[str, Any], override: Dict[str, Any]) -> None:
        ...
        for key, value in override.items():
            if ConfigManager._is_nested_dict(base, key, value):
                ConfigManager._merge_dicts(base[key], value)
            else:
                base[key] = value

    @staticmethod
    def _is_nested_dict(base: Dict[str, Any], key: str, value: Any) -> bool:
        return (
            key in base
